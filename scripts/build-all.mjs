...
function imageBuild() {
  if (isWin) info('Windows image build runs under WSL2 (docker lives inside WSL, not the Windows PATH).');
  if (buildFullImage) {
...
// Windows analog of desktopLauncher(): a PowerShell run.ps1 staged at build/<config>/. Points the
// packaged app at THIS tree's VM bundle via ATELIER_BUNDLE_DIR (wins over the app's packaged
// default) and launches it. The broker is started separately, elevated (see summary).
function desktopLauncherWin() {
  return `#requires -version 5
# Launch the packaged Atelier desktop app against the VM image bundle in this build tree.
# Generated by scripts/build-all.mjs — re-run \`npm run build:all\` to refresh; don't hand-edit.
# Start the broker yourself first, elevated, e.g.:  .\\atelierd.exe -addr \\\\.\\pipe\\atelierd
$ErrorActionPreference = 'Stop'

$here   = Split-Path -Parent $MyInvocation.MyCommand.Path
$bundle = Join-Path $here 'image\\${target}'

if (-not (Test-Path (Join-Path $bundle 'vmlinuz'))) {
  Write-Error "atelier: VM image not found at $bundle — run: npm run build:all"; exit 1
}

$app = Get-ChildItem (Join-Path $here 'desktop') -Filter 'Atelier-win32-*' -Directory -ErrorAction SilentlyContinue |
       ForEach-Object { Join-Path $_.FullName 'Atelier.exe' } |
       Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $app) {
  Write-Error "atelier: packaged app not found under $here\\desktop — run: npm run build:all"; exit 1
}

$env:ATELIER_BUNDLE_DIR = $bundle
& $app @args
exit $LASTEXITCODE
`;
}

// Double-clickable shim next to run.ps1: invokes it with execution policy bypassed (so a locked-down
// machine still launches), preferring pwsh 7 but falling back to the always-present Windows PowerShell.
function desktopLauncherCmdWin() {
  return `@echo off
rem Generated by scripts/build-all.mjs — re-run \`npm run build:all\` to refresh; don't hand-edit.
setlocal
set "PS=powershell"
...
function desktop() {
  ...
  const buildDir = rel('build', flags.config);
  fs.mkdirSync(buildDir, { recursive: true });
  if (isMac) {
    const launcher = path.join(buildDir, 'run.sh');
    ...
    fs.writeFileSync(path.join(buildDir, 'run.ps1'), desktopLauncherWin());
    fs.writeFileSync(path.join(buildDir, 'run.cmd'), desktopLauncherCmdWin());
    info(`run.ps1 + run.cmd -> launches packaged desktop against image/${target}`);
  }
}
...
function summary() {
  ...
  if (isWin)
    console.log(`    build\\${flags.config}\\run.cmd                                            # desktop (packaged)`);
}
